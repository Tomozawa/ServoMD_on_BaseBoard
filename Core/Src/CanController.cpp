...
namespace servo_md {
	...
	template<typename T>
	CanController<T>::CanController(CanManager& can_manager, Parameters& params, uint32_t offset_from_bid): can_manager(can_manager), params(params), offset_from_bid(offset_from_bid){
	...
	void CanController<T>::update(void){
		...
		while(ite != rx_frames.end()){
			if(ite->header.get_id() == (params.get_BID() + offset_from_bid)){
				callback(CRSLib::Can::unpack<std::endian::native, T>(ite->data), ite->header.get_id());
